...
We are improving our API user experience by migrating ``sellerReferenceNumber`` as a searchable field throughout the API.
...
Meanwhile, all of our ``<xxx>Number`` fields
(e.g. ``returnRequestNumber``, ``shipmentNumber``, ``returnRequestLineItemNumber``)
will be updated as non-assignable fields. Meaning that these fields will be generated by Return Helper system and cannot be assigned by API users.
...
- all existing API integrations can still use the old ``<xxx>Number`` fields

**Migration completed** (from 1st March 2025)
- all above changes will be enforced, all API requests must use the new ``sellerReferenceNumber`` field
...
- all ``<xxx>Number`` values in API requests are automatically transferred to the new ``sellerReferenceNumber`` field
- all ``<xxx>Number`` fields will be generated by Return Helper system
- Either ``<xxx>Number`` or ``sellerReferenceNumber`` can be used in the request, otherwise the system will return a validation error
- All notifications will include both ``<xxx>Number`` and ``sellerReferenceNumber`` fields, with the same value of ``sellerReferenceNumber``
...
we replace the value of ``<xxx>Number`` with ``sellerReferenceNumber`` field in our notifications.
...
Every API request must use the new ``sellerReferenceNumber`` field.
